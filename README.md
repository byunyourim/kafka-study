...
# acks
#### acks=0
동작: 서버의 응답을 전혀 기다리지 않고, 메시지를 소켓 버퍼에 추가하자마자 전송 완료로 간주  
   
서버가 메시지를 수신했는지 보장할 수 없으며, 오류 발생 여부를 알 수 없어 retries 설정이 적용되지 않습니다.
반환되는 오프셋(offset)은 항상 -1로 설정됩니다. 
- 장점: 빠른 전송 속도
- 단점: 데이터 손실 가능성 매우 큼

#### acks=1
동작: 리더(leader)가 메시지를 로컬 로그에 기록한 후 바로 응답  
   
팔로워(follower) 리플리카의 복제 완료를 기다리지 않음
리더가 메시지를 기록 후 장애가 발생하면 복제되지 않아 데이터 손실 가능
- 장점: 상대적으로 빠른 처리 속도
- 단점: 리더 장애 시 데이터 손실 위험

#### acks=all (또는 acks=-1)
동작: 리더가 모든 **in-sync 리플리카(동기화된 복제본)**의 응답을 받은 후 전송 완료로 간주

최소 하나 이상의 in-sync 리플리카가 살아 있는 한 데이터 손실 방지하며, 가장 강력한 내구성 보장합니다.
- 장점: 데이터 안전성 극대화
- 단점: 높은 지연(latency) 및 네트워크 부하 증가

#### Idempotence (중복 방지)
중복 방지 기능으로, 이 기능을 활성화하려면 반드시 acks=all이어야 합니다.
이 설정이 충돌하거나 명시적으로 중복 방지가 활성화되지 않은 경우, 기본적으로 중복 방지는 비활성됩니다.

---
# Replication

데이터를 안전하게 저장하기 위해 하나의 메시지를 여러 브로커에 복제(replication)합니다.
리더(leader) 브로커가 중심 역할을 하고, 팔로워(follower) 브로커들이 리더의 데이터를 복제합니다.

#### 설정 예시
리플리케이션 팩터 = 3   
→ 메시지가 3개의 브로커(리더 1 + 팔로워 2)에 복제됩니다.   
min.insync.replicas = 2    
→ 메시지를 최소 2개의 브로커(리더 포함)가 수신해야 성공으로 간주합니다.
